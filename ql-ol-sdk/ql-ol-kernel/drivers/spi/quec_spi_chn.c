...
static ssize_t spidev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos) {
    ...
    if (count > kfifo_len(&ch_info->rx_fifo))
        count = kfifo_len(&ch_info->rx_fifo);

    if(kfifo_to_user(&ch_info->rx_fifo, buf, count, &copied))
    {
        mutex_unlock(&ch_info->rx_mutex);
        return -EFAULT;
    }
    ...
    if (debug & 0x01) dev_info(ch_info->dev, "%s copied = %d, %d / %d\n", __func__, copied,
...
static ssize_t spidev_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos) {
    ...
    if (count > kfifo_avail(&ch_info->tx_fifo))
        count = kfifo_avail(&ch_info->tx_fifo);

    if(kfifo_from_user(&ch_info->tx_fifo, buf, count, &copied))
    {
        mutex_unlock(&ch_info->tx_mutex);
        return -EFAULT;
    }
