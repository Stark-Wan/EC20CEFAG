...
//add by dawn 2018-11-5
static unsigned int write_forbidden = 0;//the flag to close mtd write and erase
				...
				struct mtd_oob_ops *ops)
{
    ...
    /*
     *add by Dawn 2018-11-5
     *if set write_forbidden=1 the mtd will be forbidden write!
     */
    if(write_forbidden == 1) {
        printk("[Dawn] %s: MTD write forbidden flag = %d\n", __func__, write_forbidden);
        return -1;
    }
...
/*
*add by Dawn 2018-11-5
*forbidden mtd write data to nand flash by echo 1 > /sys/devices/7980000.nand/write_forbidden
*/
static ssize_t mtd_write_forbidden_store(struct device *dev, struct device_attribute *attr, char *buf, size_t count)
{
    int retval = 0;
    retval = kstrtouint(buf, 0, &write_forbidden);
    return count;
}

static ssize_t mtd_write_forbidden_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return snprintf(buf,PAGE_SIZE,"%u\n",write_forbidden);
}
static DEVICE_ATTR(write_forbidden, S_IRUGO|S_IWUSR, mtd_write_forbidden_show, mtd_write_forbidden_store);
...
static int msm_nand_probe(struct platform_device *pdev)
{
    ...
    */
    ...
    printk("[Dawn] %s: create attr file mtd_write_forbidden start\n", __func__);
    if ((ret = device_create_file(&pdev->dev, &dev_attr_write_forbidden)) != 0)
    {
        printk("%s: create attr file mtd_write_forbidden failed\n", __func__);
        //goto free_bam;
    }
