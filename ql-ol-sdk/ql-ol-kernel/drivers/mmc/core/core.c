...
/*add for debug
 if(mrq->sbc){
    printk("sbc %s: cmd%u, arg: %08x, resp:%08x%08x%08x%08x \n",mmc_hostname(host), mrq->sbc->opcode,mrq->sbc->arg,mrq->sbc->resp[0],mrq->sbc->resp[1],mrq->sbc->resp[2],mrq->sbc->resp[3]);
 }
 printk("%s: cmd%u, arg: %08x, resp:%08x%08x%08x%08x \n",mmc_hostname(host),cmd->opcode,cmd->arg,cmd->resp[0],cmd->resp[1],cmd->resp[2],cmd->resp[3]);
 if(mrq->stop){
    printk("stop %s: cmd%u, arg: %08x, resp:%08x%08x%08x%08x \n",mmc_hostname(host), mrq->stop->opcode,mrq->stop->arg,mrq->stop->resp[0],mrq->stop->resp[1],mrq->stop->resp[2],mrq->stop->resp[3]);
 }
*/
