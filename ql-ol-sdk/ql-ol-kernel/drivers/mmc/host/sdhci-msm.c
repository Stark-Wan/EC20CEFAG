...
//add by javen 2018-12-15 add nisr sdio interrupu atttibute file
extern u32 sdhci_irq_int_status;
static ssize_t show_nisr(struct device *dev, struct device_attribute *attr, char *buf)
{
        //readl_relaxed(host->ioaddr + SDHCI_INT_STATUS);
        return snprintf(buf, PAGE_SIZE, "0x%x\n", sdhci_irq_int_status);
}
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (ret) {
        ...
        //add by javen 2018-12-15 add nisr sdio interrupu atttibute file
        msm_host->nisr.show = show_nisr;
        sysfs_attr_init(&msm_host->nisr.attr);
        msm_host->nisr.attr.name = "nisr";
        msm_host->nisr.attr.mode = 0444;
        ret = device_create_file(&pdev->dev, &msm_host->nisr);
        if (ret) {
                pr_err("%s: %s: failed creating nisr attr: %d\n",
                mmc_hostname(host->mmc), __func__, ret);
                device_remove_file(&pdev->dev, &msm_host->nisr);
        }
