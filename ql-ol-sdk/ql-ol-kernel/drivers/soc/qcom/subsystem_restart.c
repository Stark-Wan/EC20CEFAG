...
/* add by [jared.lu] 20180515 for add aprstlevel default value*/
#define QUECTEL_SYSCONFIG_APRSTLEVEL
#ifdef QUECTEL_SYSCONFIG_APRSTLEVEL

static int __init quectel_set_aprstlevel(char *str)
{
	unsigned int rstlevel;
		
    pr_info("[Dawn] aprstlevel str %s\n", str);
	
	kstrtouint(str, 0, &rstlevel);

	system_reset_mode = rstlevel;

	pr_info("[Dawn] system_reset_mode %d\n", system_reset_mode);
	
    return 0;
}

early_param("aprstlevel", quectel_set_aprstlevel);
...
//2018-12-13:javen:fix the compile error 
//#endif
/* add by [jared.lu] 20180515 for add modemrstlevel default value*/
static int modem_reset_mode = 0; //0 - dload, 1 - reset

/* add by [jared.lu] 20180515 for add modemrstlevel default value*/
#define QUECTEL_SYSCONFIG_MODEMRSTLEVEL
#ifdef QUECTEL_SYSCONFIG_MODEMRSTLEVEL

static int __init quectel_set_modemrstlevel(char *str)
{
	unsigned int rstlevel;
	
    pr_info("[Dawn] modemrstlevel str %s\n", str);

	kstrtouint(str, 0, &rstlevel);

	modem_reset_mode = rstlevel;
	
	pr_info("[Dawn] modemrstlevel %d\n", modem_reset_mode);
	
    return 0;
}

early_param("modemrstlevel", quectel_set_modemrstlevel);

#endif


static int dload_mode_mode=0;
void msm_set_restart_mode(int mode);
int msm_get_restart_mode(void);

//matthew.ma-2018/06/13:add a node to enter sbl download(qpst)
static ssize_t
system_dload_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
		dload_mode_mode=msm_get_restart_mode();
        return snprintf(buf, PAGE_SIZE, "%d\n", dload_mode_mode);
		
}

static ssize_t system_dload_mode_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
	sscanf(buf, "%d", &dload_mode_mode);
	msm_set_restart_mode(dload_mode_mode);
    return count;
}
...
static struct device_attribute subsys_attrs[] = {
	...
	__ATTR(system_dload_mode, 0644, system_dload_mode_show, system_dload_mode_store),
#endif
	__ATTR_NULL,
};
...
struct subsys_device *subsys_register(struct subsys_desc *desc)
{
	...
	init_waitqueue_head(&quectel_check_modem_state_wait);
        ...
        //add by dawn 2018-9-14
		subsys->restart_level = modem_reset_mode;
    	}
#endif
#endif

	/*add by dawn 2018-7-27
	*to repair at+qcfg="ApRstLevel",0 and at+qcfg="ApRstLevel",0 can not crasp the dump. 
	*/
	quectel_set_system_reset_mode((system_reset_mode == 0) ? 1 : 0);
