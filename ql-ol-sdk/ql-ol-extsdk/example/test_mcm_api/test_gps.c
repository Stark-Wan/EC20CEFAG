                                 ...
                                 void                    *contextPtr)
{
    ...
    switch(e_msg_id)
    {
        ...
        case E_QL_LOC_NFY_MSG_ID_SV_INFO:
        {
            ...
            printf("Total %d SV, ephemeris_mask=0x%X, almanac_mask=0x%X, used_in_fix_mask=0x%X, detail infor:\n", 
                    pt_svinfo->num_svs,
                    pt_svinfo->ephemeris_mask,
                    pt_svinfo->almanac_mask,
                    pt_svinfo->used_in_fix_mask); 
            for(i=0; i<pt_svinfo->num_svs; i++)
            {
                printf("\tSV[%d]: prn=%d, snr=%f, elevation=%f, azimuth=%f, ephemeris[%c], almanac[%c], used_in_fix[%c]\n",
                    i, 
                    pt_svinfo->sv_list[i].prn,
                    pt_svinfo->sv_list[i].snr,
                    pt_svinfo->sv_list[i].elevation,
                    pt_svinfo->sv_list[i].azimuth,
                    (pt_svinfo->ephemeris_mask  &(1<<i)) ? 'V':'X', //have ephemeris data
                    (pt_svinfo->almanac_mask    &(1<<i)) ? 'V':'X', //have almanac data
                    (pt_svinfo->used_in_fix_mask&(1<<i)) ? 'V':'X');//used for computing the most recent position fix
            }
            break;
        }
        case E_QL_LOC_NFY_MSG_ID_NMEA_INFO:        
        {
            ...
            printf("NMEA info: timestamp=%lld(%04d:%02d:%02d, %02d:%02d:%02d), length=%d, nmea=%s\n", 
                    pt_nmea->timestamp, 
                    utc->tm_year + 1900, 
                    utc->tm_mon + 1, 
                    utc->tm_mday, 
...
static int test_gps(void)
{
    ...
    while(1)
    {
        ...
        case 16://"QL_LOC_Agps_SetServer"
        {
            ...
            printf("please input AGPS type(0:ANY, 1:SUPL, 2:C2K, 3:WWAN_ANY, 4:WIFI, 5:SUPL_ES): \n");
            scanf("%d", &tmp);
            t_info.e_agps_type = tmp;

            printf("please input server name: ");
            scanf("%s", t_info.host_name);
