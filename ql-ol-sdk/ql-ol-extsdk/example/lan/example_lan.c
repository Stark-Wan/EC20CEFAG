...
int main(int argc, char **argv)
{
    ...
    while(1) {
        ...
        printf("\ncase 3: Set Ethernet Tethering Mode");
        printf("\ncase 4: Get Ethernet Tethering Mode");
        printf("\ncase 100: Exit");
        ...
        switch(atoi(sptr)) {
            ...
            case 3:
            {
                ql_ethernet_mode_e eth_mode = QL_ETHERNET_MODE_LAN_ROUTE;
                printf("\nPlease intput Ethernet Tethering Mode(0-LAN Route/1-WAN Route):");
                scanf("%s",sptr);
                if(0 != atoi(sptr) && 1 != atoi(sptr)) {
                    printf("\nInvalid Ethernet Tethering Mode");
                    break;
                }
                eth_mode = 0 == atoi(sptr) ? QL_ETHERNET_MODE_LAN_ROUTE : QL_ETHERNET_MODE_WAN_ROUTE;
                if(QL_Ethernet_Mode_Set(eth_mode)) {
                    printf("\nSet Ethernet Tethering Mode failure");
                }
                break;
            }
            case 4:
            {
                ql_ethernet_mode_e eth_mode = QL_ETHERNET_MODE_LAN_ROUTE;
                if(QL_Ethernet_Mode_Get(&eth_mode)) {
                    printf("\nSet Ethernet Tethering Mode failure");
                    break;
                }
                printf("\nEthernet Tethering Mode: %s",
                    eth_mode == QL_ETHERNET_MODE_LAN_ROUTE ? "LAN ROUTE" : "WAN ROUTE");
                break;
            }
            case 100:
            {
